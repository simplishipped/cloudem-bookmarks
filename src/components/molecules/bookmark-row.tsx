...
export const BookmarkRow: Component<BookmarkProps> = (props) => {
  ...
  return (
    ...
    btn-hover-one cursor-default fill-textLight dark:fill-textDark border-textLight dark:border-textDark  rounded-3xl">
      ...
      <div class="flex justify-between items-center">
        <div onClick={() => setDropdown(dropdown() ? false : true)} class="flex items-center cursor-pointer">
          ...
          <div class={`${props.row().temporary ? 'text-yellow-500 dark:text-yellow-500' : ''} ml-2 transition-all`}>{props.row().name.slice(0, 23) + (props.row().name.length > 23 ? '...' : '')}</div>
    ...
    dark:text-textDark border-b-2 border-b-primaryButtonLight dark:border-b-primaryButtonDark pb-1">{props.row().url.slice(0, 40) + (props.row().url.length > 40 ? '...' : '')}</div>
        </div> : false}
    </div>
  );
};
